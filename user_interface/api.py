...
def assay_request_endpoint():
    ...
    GET request:
        Renders the blank assay request creation page.
    POST request:
        Parses the fields and values specified in the form into a JSON document, inserts the document into the assay_requests database collection, and renders a new blank assay request creation page with a message providing the new database ID of the document that was just added to the database.
        form data:
            * grouping (str): experiment name
            * sample.name (str): concise sample description
            * sample.description (str): more detailed sample description
    ...
    if request.method == "POST":
        new_doc_id, error_msg = perform_insert(request.form, db_obj, coll_type='assay_requests')
...
def assay_request_search_endpoint():
    """Intakes query term elements, assembles them into a valid query term, and appends the new query term to an existing query, if possible. If the option to perform the search is selected, this endpoint calls the back-end's search function with the newly assembled query and returns the resulting documents from the assay_requests collection of the database.

    GET request:
        Render the assay request search page with no results or existing query.
    POST request:
        Render the assay requets search page with specific information depending on the form elements present:
        form data:
            * append_button (str): if the value for this field is "do_and" or "do_or", the rest of the form data is parsed into the elements of a new query term and added to whatever query terms already exist. If the value is "do_and", the query term is added to the existing query with an "and" operation. If the value is "do_or", it is added with an "or" operation.
            * existing_query (str): the human-readable version of whatever existing query is already present and is currently being added to.
            * query_field (str): the field that is being queried against.
    ...
    if request.form.get("append_button") == "do_and":
        ...
        results, error_msg = perform_search(final_q, db_obj, coll_type='assay_requests')
...
def assay_request_update_endpoint():
    """Finds the assay request document with the given ID in the database and updates its fields and values with the fields and values that the user supplies in the form data. If the button to verify the doc is clicked, the document is updated with the given values and moved to the main assays collection as a finished assay request. If, instead, the button to update the dock is clicked, the document original document is moved to the assay request old versions collection and an doc that is updated with the given values is inserted into the assay requests database. 

    GET request:
        Renders the page for a user to specify a document ID to search for.
    POST request:
        If the "submit_button" form data value is "find_doc", extract the "doc_id" value, search for it in the database, and return the update page with the found document's fields and values. If the "submit_button" value is "update_doc", extract all the new user-specified fields and values and update the document in the database with them. If the "submit_button" value is "verify_doc", move the updated doc into the main assays database as a finished assay request.
        form data:
            * submit_button (str): one of "find_doc", "update_doc", or "verify_doc", this field dictates what operation to perform on the back-end
            * doc_id (str): this should only be present if the "find_doc" option is specified
            * grouping (str): the updated value for experiment name
            * sample.name (str): the updated value for concise sample description
            * sample.description (str): the updated value for more detailed sample description
    ...
    if request.method == "GET":
        ...
        doc_id = request.form.get('doc_id', '')
        doc = search_by_id(doc_id, db_obj, coll_type='assay_requests')
        ...
        doc_id, remove_doc, update_pairs, meas_remove_indices, meas_add_eles = parse_update(request.form)
        new_doc_id, error_msg = perform_update(doc_id, remove_doc, update_pairs, meas_remove_indices, meas_add_eles, db_obj, is_assay_request_update=is_update, is_assay_request_verify=is_verify)
        if new_doc_id != None:
